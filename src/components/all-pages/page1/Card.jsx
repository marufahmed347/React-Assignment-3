...
const CardSection = () => {
  ...
  return (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {restaurants.map((restaurant, index) => (
          ...
          onClick={() => handleCardClick(index)}
            className={`overflow-hidden relative bg-[#F8F8F8] rounded-lg ${
              index === 0
    ...
    <div className="lg:pb-10 px-5 lg:px-10 pb-5 lg:pt-5 pt-3">
      ...
      <button onClick={() => navigate('/page2')} className="px-8 py-2 bg-[#1677BD] text-white rounded-full cursor-pointer block mx-auto">Show more</button>
    </div>
    </>
  );
};
